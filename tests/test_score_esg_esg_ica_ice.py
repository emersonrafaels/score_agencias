...
from src.calculator_score.esg.esg.score_ica_ice import ICA, ICE
...
if __name__ == "__main__":
    """
    Função principal que executa testes individuais de cálculo de score para ICA.
    """

    test_cases = [
        (1.2, 7.67, 0.0, 10.0, 2),  # Teste padrão
        (1.3, 7.123, 7.0, 7.5, 3),  # Teste com arredondamento específico
    ]

    for case in test_cases:
        indice, score_esperado, minimo, maximo, casas_decimais = case
        try:
            test_ica_calculo_score(
                indice, score_esperado, minimo, maximo, casas_decimais
            )
